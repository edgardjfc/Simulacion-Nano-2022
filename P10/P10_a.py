...
def weightsGenerator(amount, low, high):
    return np.round(normalize(np.random.uniform(low=low, high=high, size = amount)) * (high - low) + low)
 
def valuesGenerator(weights, low, high):
    return np.round(normalize(np.random.uniform(low = low, high = high, size = weights)) * (high - low) + low)
 
def initialPopulation(n, size):
    pobl = np.zeros((size, n))
    for i in range(size):
